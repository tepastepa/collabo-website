...
function IconBox({ name, color, light = false }) {
  const Icon = Icons[name];
  if (!Icon) return null;
  return (
    <div className={light ? "icon-box icon-box-light" : "icon-box"} style={color ? { color } : undefined}>
...
function ClosingBand({ product }) {
  ...
  if (product.pricing) {
    return (
      <section className="bg-[var(--p-deep)] text-white">
        <div className="container flex flex-col items-center py-20 text-center md:py-24">
          <p className="t-label uppercase tracking-widest text-white/60">{product.pricingTitle}</p>
          ...
          <div className="container flex flex-col items-center pt-20 pb-0 text-center md:pt-24">
            ...
            {hasHangingImage && (
              <div className="-mb-16 mt-12 w-full overflow-hidden rounded-t-2xl border-x border-t border-white/10 md:-mb-24">
                <img src={product.image} alt={`Интерфейс ${product.name}`} className="w-full" />
              </div>
            )}
                  ...
                  <div key={step.text} className="grid items-center gap-8 md:grid-cols-2 md:gap-16">
                    <div className={i % 2 === 1 ? "md:order-2" : ""}>
                      <div className="flex items-start gap-5">
                        ...
                        <p className="max-w-xs pt-2 t-heading text-[var(--color-ink)]">{step.text}</p>
                      </div>
                    </div>
                    <div className={`mx-auto w-full max-w-[300px] overflow-hidden rounded-3xl border border-[var(--color-ink)]/10 bg-white shadow-[0_24px_60px_-30px_rgba(0,0,0,0.25)] ${i % 2 === 1 ? "md:order-1" : ""}`}>
        ...
        {product.features && (
          <section className="border-t border-[var(--color-ink)]/10">
            <div className="container py-20 md:py-28">
              ...
              <div className={`mt-14 grid gap-4 ${product.features.length <= 4 ? "sm:grid-cols-2" : "sm:grid-cols-2 lg:grid-cols-3"}`}>
          ...
          <section key={s.title} className="border-t border-[var(--color-ink)]/10">
            <div className="container grid items-center gap-10 py-20 md:grid-cols-2 md:gap-16 md:py-28">
              ...
              <div className={i % 2 === 1 ? "md:order-1" : ""}>
                ...
                <p className="mt-5 max-w-md t-body text-[var(--color-muted)]">{s.text}</p>
              </div>
            </div>
          </section>
        ))}
